...
{ 
    ...
    for(int i = 0; i < vertices.size(); i++)
    {
        ...
        // Update velocities using the forces (since mass m = 1)
        vertices[i].vx += vertices[i].fx * tstep;
        vertices[i].vy += vertices[i].fy * tstep;

        // Update positions using the new velocities
        vertices[i].x += vertices[i].vx * tstep;
        vertices[i].y += vertices[i].vy * tstep;
