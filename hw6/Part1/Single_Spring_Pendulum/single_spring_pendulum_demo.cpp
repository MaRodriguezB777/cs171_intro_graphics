...
void update_pendulum()
{
    ...
    float r = sqrt(m1.x * m1.x + m1.y * m1.y);

    // Avoid division by zero
    if (r == 0.0f)
        r = 0.0001f;

    // Compute the spring force components
    float Fx = -m1.k * (r - m1.rl) * (m1.x / r);
    float Fy = -m1.k * (r - m1.rl) * (m1.y / r);

    // Add gravitational force to the y-component
    Fy += m1.m * g;
