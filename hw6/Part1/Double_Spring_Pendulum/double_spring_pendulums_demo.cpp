...
void update_pendulums()
{
    ...
    float s1 = sqrt(m1.x * m1.x + m1.y * m1.y);
    float s2 = sqrt((m2.x - m1.x) * (m2.x - m1.x) + (m2.y - m1.y) * (m2.y - m1.y));

    // Avoid division by zero
    if (s1 == 0) s1 = 0.0001f;
    if (s2 == 0) s2 = 0.0001f;

    // Compute the forces on m1
    float F1x_spring1 = -m1.k * (s1 - m1.rl) * m1.x / s1;
    float F1y_spring1 = -m1.k * (s1 - m1.rl) * m1.y / s1;

    float F1x_spring2 = m2.k * (s2 - m2.rl) * (m2.x - m1.x) / s2;
    float F1y_spring2 = m2.k * (s2 - m2.rl) * (m2.y - m1.y) / s2;

    float F1x = F1x_spring1 + F1x_spring2;
    float F1y = F1y_spring1 + F1y_spring2 + m1.m * g;

    // Compute the forces on m2
    float F2x = -m2.k * (s2 - m2.rl) * (m2.x - m1.x) / s2;
    float F2y = -m2.k * (s2 - m2.rl) * (m2.y - m1.y) / s2 + m2.m * g;
