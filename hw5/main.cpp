...
void print_parsed_scene(Scene& scene, std::unordered_map<std::string, Object *> *objects) {
    ...
    for (auto& shape : *scene.shapes) {
        ...
        bool first = true;
        // for (int i = 1; i < shape->mesh_data->vertices->size(); i ++) {
        //     Vertex *p = shape->mesh_data->vertices->at(i);
        //     Vector3f v(p->x, p->y, p->z);
        //     std::cout << v.transpose() << std::endl;
        // }
        // std::cout << "Shape faces: " << std::endl;
        // for (int i = 0; i < shape->mesh_data->faces->size(); i++) {
        //     Face *f = shape->mesh_data->faces->at(i);
        //     Vector3f v(f->idx1, f->idx2, f->idx3);
        //     std::cout << v.transpose() << std::endl;
        // }
...
Scene read_scene_file(std::string filename) {
    ...
    std::unordered_map<std::string, Object *> *objects = new std::unordered_map<std::string, Object *>();
    ...
    while(std::getline(file, line)) {
        ...
        if (type == "camera:") {
            ...
            float k;
            iss >> k;
            lights->push_back(new Light(x, y, z, r, g, b, k));
        } else if (type == "objects:") {
            read_objects_section(file, objects, directory);
        } else if (objects->find(type) != objects->end()) {
            Object *obj = objects->find(type)->second;
            shapes->push_back(new Shape(create_shape(obj->name, obj->filename, file, *objects->at(type))));
        } else {
...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " [scene_description_file.txt] [xres] [yres] [h] [shading_type [optional]]" << std::endl;
        exit(1);
    }
